...
bool handleFileRead(String path) { // send the right file to the client (if it exists)
  ...
  if (SPIFFS.exists(path)) { // If the file exists
    ...
    if (path == "/data.txt") {
      if (data >= 500) {
        text = (String)data + " - LED ON"; // Convert sensor data to string
        digitalWrite(2, LOW);
        server.send(200, "text/plain", text);
      }
      else if (data < 500) {
        text = (String)data + " - LED OFF"; // Convert sensor data to string
        digitalWrite(2, HIGH);
        server.send(200, "text/plain", text);
      }
    }
    size_t sent = server.streamFile(file, contentType); // The missing line of code
